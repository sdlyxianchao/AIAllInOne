...
## 1. Positioning
Keycloak is the platform's **identity & authentication hub**: the SSO-wired products (AI Admin Center, NewAPI, Dify, Gitea, Grafana, Langfuse, LiteLLM) all use it for Single Sign-On (SSO); Ghost and the non-web tools (DSH Desktop, MCP Gateway, Update Server, MailHog, Ollama) are **not** wired into SSO. Mastering Keycloak is a prerequisite for deploying and operating this platform.
