...
**Positioning**: centralized Skill/MCP tool gateway. DeepChat and Dify connect to one address and get all tools (built-in platform tools + aggregated external MCP servers + Dify knowledge retrieval RAG), and it hosts the intranet **Skill Marketplace**. The hub for "AI capability extension".
...
**Prereq**: M06 (KB + Knowledge API key), M09 (DeepChat MCP).
...
**Pass**: DeepChat → :3100/mcp → search_knowledge hits KB; upload/delete a Skill; add/remove an external MCP server; explain the 3 RAG pitfalls.
...
**1. What MCP is**: Model Context Protocol (Anthropic) — the "USB-C for AI": unified interface for LLMs ↔ external tools/data. Architecture: Host (DeepChat/Dify) ↔ Client ↔ Server (tools/resources/prompts). This gateway is **one unified MCP Server**.
...
```
- DeepChat: manual config → Streamable HTTP → `http://<SERVER_IP>:3100/mcp` (M09).
...
- Custom built-in: add tool def to `builtinTools` + branch in `callBuiltin` in gateway.js → restart → DeepChat reconnects.
...
- Step 3: `docker compose up -d mcp-gateway` → DeepChat calls `search_knowledge` (query, optional dataset_id/top_k) → returns segments (content + score); or Admin Center → Dify page → RAG card.
- ⚠️ 3 pitfalls:
...
- Alternative: DeepChat's built-in `difyKnowledge` MCP (endpoint with /v1, apiKey, datasetId) — but keys live on every client (account-level, hard to govern); **enterprise path is the gateway**.
...
- Built-ins: `platform-report`, `skill-market`. Install in DeepChat: Install from URL → `http://<SERVER_IP>:3100/skills/<name>.zip`.
...
| new tool not visible | reconnect MCP in DeepChat |
...
**Lab checklist**: /health + tools/list with 4 built-ins (S); DeepChat calls platform_time/services; external MCP server added (prefixed tools visible) (S); custom platform_health added; Skill uploaded & installed from URL (S); .env DIFY_* configured + restart; search_knowledge returns KB content (S); admin API add/remove a server.

**Homework**: draw DeepChat→Gateway→Dify RAG diagram with 3 pitfalls; read ch20-ops-mcp.md → 5 ops points; design an MCP server for an internal API.
...
**Defense (20)**: "Expose an internal business API to DeepChat?"; "KB retrieval returns nothing — possible causes?"; "MCP vs Skill — when to use which?"
