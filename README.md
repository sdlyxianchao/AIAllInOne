...
# Linux (Debian/Ubuntu)
cd AIAllInOne/linux
# See linux-deploy-guide-debian.en.html for full instructions

# Linux (RHEL/CentOS/Rocky)
cd AIAllInOne/linux
# See linux-deploy-guide-rpm.en.html for full instructions
```

> **Platform status**: Windows (Windows 11 + Docker Desktop) and **Linux** (Debian 12+ / Ubuntu 22.04+ / RHEL 9+ / Rocky 9+ / Fedora 39+) are **actively tested**. Online-server (`docker/`) is planned — see the [Roadmap](#roadmap).
...
- Complete Linux deployment support: Debian-based (Debian 12+ / Ubuntu 22.04+) and RPM-based (RHEL 9+ / CentOS Stream 9+ / Rocky 9+ / Fedora 39+)
- Pre-built Docker image packages for offline deployment (6.2GB main + Dify)
- Linux bash scripts: backup.sh, restore.sh, health-check.sh, ghost-setup.sh, dify-setup.sh
- Import scripts for both Windows (PowerShell) and Linux (bash)

**New: Dify AI Application Platform**
- Dify standalone deployment support with separate Docker Compose
...
- Windows deploy guide v2: added offline image import, scheduled tasks, port quick reference, verified fix log
- Linux deploy guide (Debian/RPM): full parity with Windows guide
- All guides now include Keycloak IdP questionnaire (7 IdP types) and Admin REST API auto-configuration
- Dify auto-setup and Gitea sync import steps added to all guides

**Improved: AI Admin Center**
- Availability tests no longer hardcode DeepSeek model — dynamically queries available models via NewAPI
- Works with any LLM backend (DeepSeek, OpenAI, Claude, local Ollama, etc.)
...
2. **Install it** — copy the folder into your agent's skills directory (WorkBuddy: `~/.workbuddy/skills/ai-all-in-one-deploy-ops/`; other agents follow their own skill-folder convention):
   ...
   ```
3. **Use it** — open the agent in your deployment directory and just ask, e.g. *"Run the health check"*, *"Back up the platform"*, *"Why is Ghost down?"*, *"Publish v1.00"*. The skill reads `.env` for credentials itself — you never paste passwords, and it adapts to whichever machine you point it at.
