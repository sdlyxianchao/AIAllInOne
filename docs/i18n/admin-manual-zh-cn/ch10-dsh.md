# 第10章：DSH Desktop 分发与 CI/CD

*第一部分 · 部署篇*

> 把 DSH Desktop 安装包分发给员工，以及用 Gitea Actions 自动同步官方新版本。
...
分发链路 = GitHub Releases 安装包 → `dsh-sync` 仓库的 Gitea Actions → 更新服务器（:8091）→ Ghost 下载页 → 员工下载。

> 📌 已删除 `dsh` 源码 mirror 仓库——mirror 只同步 git 源码、不同步 release 安装包，对分发无用。若要做源码审计/二次开发再单独建。
...
验证：`curl -I http://<服务器IP>:8091/dsh/dsh-desktop-windows-x64-setup.exe` → 200/206。再更新 Ghost 下载页（见第 9 章）。

## 10.3 自动同步（Gitea Actions，推荐）

| 组件 | 说明 |
| --- | --- |
| `dsh-sync` 仓库 | 普通仓库（不能用 mirror），放 `.gitea/workflows/sync.yml` + `update_ghost.py` |
...
官网 `www.dshdesktop.com` 下载页的安装包仍指向 GitHub，国内基本不通。真正解决靠 `sync-config.json`：
...
# 产物在 dist/，退出后 copy 到 dsh-updates/
```

## 10.6 配置 DSH Desktop 客户端（员工侧）

1. DSH Desktop → 设置 → 模型服务 → 自定义 Provider / OpenAI 兼容；
