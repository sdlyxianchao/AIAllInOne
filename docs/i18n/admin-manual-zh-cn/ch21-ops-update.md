...
> DSH Desktop 安装包托管与自动更新；AI 管理中心可查看文件与版本。
...
- **安装包列表**：分页查看 `dsh-updates/dsh/` 下的文件（名称/大小/更新时间）。
...
- 浏览器打开 `http://<服务器IP>:8091`（nginx 目录列表，无需登录）；数据目录 `dsh-updates/`。
...
1. 下载 DSH Desktop 官方安装包到 `dsh-updates/dsh/`；
2. 更新 `version.txt`（写入新版本号）；
3. 员工侧 DSH Desktop 自动更新时检查 `version.txt` 发现新版即下载安装。

## 21.4 自动同步（推荐，项目默认）

靠 `dsh-sync` 仓库的 Gitea Actions 每天自动检查 GitHub 新版本并同步（见第 10 章；也可以在 AI 管理中心「Gitea 源码管理」页手动触发/改定时）。手动触发：
...
> 📌 DSH Desktop 客户端报「模型连接超时」通常是客户端走了挂掉的系统代理（`ECONNREFUSED 127.0.0.1:33210`）。让用户在 DSH Desktop「设置 → 网络/代理」改为「不使用代理/直连」。
