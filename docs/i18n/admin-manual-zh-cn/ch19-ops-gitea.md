...
> 内部 Git + CI/CD：仓库、组织、Runner、Actions；deepchat-sync 同步管理可在 AI 管理中心完成。
...
- **仓库列表**：分页查看仓库（名称/描述/语言/大小/更新时间），`deepchat-sync` 仓库带 sync 标记；
- **deepchat-sync 同步管理**（项目核心）：
  - **手动触发**：点「▶️ 立即同步」跑一次 DeepChat 新版本同步（Gitea Actions）；
  - **定时计划**：选频率（每天 02:00 / 每 12h / 每 6h / 每 3h / 每小时 / 每周一 / 自定义 cron）→ 保存；
  - **同步目标与保留数**：配置同步的目标仓库与 `keep_releases`（版本保留数）；
  - **版本列表**：已同步的 DeepChat 版本清单；
...
*图 19-1：AI 管理中心「Gitea 源码管理」页（deepchat-sync 同步管理）*
...
![deepchat-sync 仓库页](../../images/admin-manual/product-gitea-repo.png)

*图 19-3：deepchat-sync 仓库页*

![Gitea Actions 运行页](../../images/admin-manual/product-gitea-actions.png)

*图 19-4：Gitea Actions 运行记录（deepchat-sync 自动同步）*
...
4. **跑工作流**：仓库 → Actions → 手动运行或 push 触发（deepchat-sync 的 `sync.yml` 每天自动跑，也可在 AI 管理中心手动触发）。
