...
> 统一管理员门户：Keycloak 鉴权、左侧菜单按「应用服务 / 平台基础设施 / 运维监控 / 系统管理」分组、Dashboard 集群状态。
...
| 应用服务 | 📰 Ghost 后台 `:8090` | 内嵌统计页 | 门户概览（文章数等）；「打开后台」自动免密登录 |
| 应用服务 | 🤖 Dify AI 平台 `:80` | 内嵌统计页 | 应用/知识库概览 + 知识库检索测试；「打开后台」跳转 |
| 应用服务 | 📦 Gitea 源码管理 `:3002` | 内嵌管理页 | 仓库列表 + dsh-sync 同步管理（触发/定时/配置/版本/历史）；「打开后台」SSO 登录 |
| 应用服务 | 🔀 NewAPI 管理 `:3000` | 内嵌统计页 | 渠道/用户/令牌概览 + 成本报表 + 审计日志 |
| 应用服务 | 🛡️ LiteLLM + PII 脱敏 `:4001` | 内嵌统计页 | LiteLLM 概览 + 模型列表；「打开后台」跳 `/ui` |
| 平台基础设施 | 🔐 Keycloak 认证 `:9090` | 内嵌管理页 | 用户/客户端/角色列表（分页+搜索）；全部/增量同步、删用户、角色管理（全局管理员） |
| 平台基础设施 | 🔌 MCP Gateway `:3100` | 内嵌管理页 | MCP Server 增删改 + 工具列表 + 技能上传/删除 |
| 平台基础设施 | ⬇️ 更新服务器 `:8091` | 内嵌统计页 | DSH Desktop 安装包文件列表 + 当前版本 |
| 运维监控 | 🩺 可用性测试 | 内嵌页 | 定时 + 手动跑全链路连通性测试 |
| 运维监控 | 📈 监控告警 + IM 通知 `:3030` | 新标签跳转 | Grafana 大盘（SSO 自动登录）；Prometheus `:9091`、Alertmanager `:9093`；告警推送到钉钉/企微/飞书 |
| 运维监控 | 🔍 LLM 可观测 `:3010` | 新标签跳转 | Langfuse（SSO 自动登录） |
| 运维监控 | 📜 统一日志（Loki） | 内嵌页 | 按容器 + 关键字 + 时间查日志 |
| 运维监控 | 💾 备份与恢复 | 内嵌管理页 | 备份列表 + 立即备份 + 一键恢复 |
| 运维监控 | 📄 报告生成 | 内嵌管理页 | 按周期生成平台报告（.md） |
| 运维监控 | 📱 桌面客户端管理 | 内嵌管理页 | DSH Desktop 客户端版本管理与分发 |
