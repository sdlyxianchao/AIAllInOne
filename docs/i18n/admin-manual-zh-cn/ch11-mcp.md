...
> 集中管理 Skill 和 MCP 工具的网关，DSH Desktop/Dify 连一个地址即可拿到所有工具。

[← 第10章：DSH Desktop 分发与 CI/CD](ch10-dsh.md) · [📖 目录](index.md) · [第12章：AI 管理中心 →](ch12-admin-center.md)
...
1. DSH Desktop：设置 → MCP → 添加服务器 → 类型「可流式传输的 HTTP」，URL `http://<服务器IP>:3100/mcp`；
...
> 📌 DSH Desktop 里 MCP 和 Skill 是两个概念：MCP 是「工具」（function calling），Skill 是「智能体技能包」（SKILL.md + 脚本）。DSH Desktop 的 Skill 没有「自定义市场 URL」，只支持文件夹/ZIP/URL 三种安装，内网分发靠「URL 安装」变相实现。

## 11.5 ⚠️ Skill 市场主机名（部署参数，必须替换）

「技能管家」读 `config.json` 的 `market_url` 请求 `/skills` 清单。两个关键点：

- **用主机名，不能用 IP**：DSH Desktop 的 agent 环境会把 IP 脱敏成 `[IP_ADDRESS_REDACTED]`，导致读不到真实地址；
