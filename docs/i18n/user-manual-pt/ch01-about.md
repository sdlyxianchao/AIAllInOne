...
```mermaid
flowchart TB
    A["📰 AI All In One Hub (portal)"] --> B["💬 DSH Desktop (conversa no desktop)<br/>instale pela central de downloads do portal"]
    A["📰 AI All In One Hub (portal)"] --> C["🤖 Dify (aplicações Web / base de conhecimento)<br/>http://IP"]
```

*Figura 1: relação entre as três entradas*

*Três entradas: o portal (Hub) é o ponto de partida; DSH Desktop e Dify são as duas ferramentas*

## 1.2 O que posso fazer com a plataforma

| O que você quer fazer | Qual usar | Onde abrir |
| --- | --- | --- |
| Conversar no dia a dia como no ChatGPT, escrever documentos, traduzir, corrigir código | 💬 DSH Desktop | Cliente de desktop (instale primeiro pelo portal) |
...
> ✅ **Resumo em uma frase**: **conversar/escrever/traduzir → DSH Desktop**; **aplicações prontas da empresa / base de conhecimento → Dify**; **achar coisas / ver avisos / baixar software → portal Hub**. As três usam a mesma conta para login.
...
- **Iniciante**: leia os capítulos 2~4 em ordem e instale o DSH Desktop para começar;
