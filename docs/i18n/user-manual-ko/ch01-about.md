...
```mermaid
flowchart TB
    A["📰 AI All In One Hub (포털)"] --> B["💬 DSH Desktop (데스크톱 대화)<br/>포털 다운로드 센터에서 설치"]
    A["📰 AI All In One Hub (포털)"] --> C["🤖 Dify (웹 앱/지식베이스)<br/>http://IP"]
```

*그림 1: 세 가지 진입점의 관계*

*세 가지 진입점: 포털(Hub)이 시작점, DSH Desktop과 Dify가 두 도구*

## 1.2 플랫폼으로 무엇을 할 수 있나

| 하고 싶은 것 | 무엇을 쓸까 | 어디서 열까 |
| --- | --- | --- |
| ChatGPT처럼 일상 대화, 문서 작성, 번역, 코드 수정 | 💬 DSH Desktop | 데스크톱 클라이언트 (먼저 포털에서 다운로드 설치) |
...
> ✅ **한 문장으로 기억**: **채팅/작성/번역 → DSH Desktop**; **회사가 만든 앱/지식베이스 → Dify**; **찾기/공지 확인/소프트웨어 다운로드 → 포털 Hub**. 세 가지 모두 같은 계정으로 로그인.
...
- **초보자**: 제2~4장을 순서대로 읽고, 먼저 DSH Desktop을 설치해 사용하세요;
