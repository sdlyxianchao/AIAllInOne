...
```mermaid
flowchart TB
    A["📰 AI All In One Hub（门户）"] --> B["💬 DSH Desktop（桌面对话）<br/>从门户下载中心安装"]
    A["📰 AI All In One Hub（门户）"] --> C["🤖 Dify（Web 应用/知识库）<br/>http://IP"]
```

*图 1：三个入口的关系*

*三个入口：门户（Hub）是起点，DSH Desktop 和 Dify 是两个工具*

## 1.2 我能用平台做什么

| 想做什么 | 用哪个 | 在哪打开 |
| --- | --- | --- |
| 像 ChatGPT 一样日常对话、写文档、翻译、改代码 | 💬 DSH Desktop | 桌面客户端（先从门户下载安装） |
...
> ✅ **一句话记住**：**聊天/写作/翻译 → DSH Desktop**；**公司搭好的应用/知识库 → Dify**；**找东西/看公告/下软件 → 门户 Hub**。三个都用同一个账号登录。

登录方式：所有产品都走 **Keycloak 统一账号**（部分支持公司 AD 域账号，即你电脑开机那套账号）。点「登录」自动跳统一登录页，输一次账号，之后各产品无需重复登录。

## 1.4 本手册怎么用

- **新手**：按顺序读第 2~4 章，先装上 DSH Desktop 用起来；
