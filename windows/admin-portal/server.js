...
app.get('/api/gitea/overview', keycloak.protect(), async (req, res) => {
  try {
    ...
    // 僵尸进度收尾：脚本被杀 / 被强制停止时不会写终态，进度文件会永远停在 status=running。
    // 不加这层判断，页面就会一直显示「正在下载安装包」、已执行时间一直涨，即使 Gitea 里那次 run
    // 早就 failure 了。这里以 run 的结论为准，把进度标记为过期（phase 换成 stale，前端就不会
    // 再把它当运行中证据）。
    if (sync_progress && sync_progress.status === 'running'
        && sync_last_run && sync_last_run.conclusion) {
      sync_progress = {
        ...sync_progress,
        status: 'stale',
        phase: 'stale',
        stale: true,
        detail: `上次任务已结束（${sync_last_run.conclusion}），进度未收尾`,
      };
    }
    res.json({
...
// act_runner 每个 job 会另起一个容器跑任务（名字是一串随机 hex，带 act_runner 相关 label）。
// 这些 job 容器**不随 gitea-runner 容器一起停**：只停 runner 的话，里面可能还在 wget 下载或
// 往 update-server 写文件，表现就是「点了强制停止，磁盘 IO 还在动」。
// 所以强制停止时按 label 把这些孤儿 job 容器一并收掉。
async function cleanupRunnerJobContainers() {
  const killed = [];
  try {
    const list = await docker.listContainers({ all: false });
    for (const c of list) {
      const name = (c.Names && c.Names[0] ? c.Names[0].replace(/^\//, '') : '');
      if (name === 'gitea-runner') continue;              // runner 本体由调用方管理
      const labels = c.Labels || {};
      const isJob = Object.keys(labels).some(k => /act_runner/i.test(k)) ||
                    Object.values(labels).some(v => /act_runner/i.test(String(v)));
      if (!isJob) continue;
      const short = name || String(c.Id).slice(0, 12);
      try {
        await docker.getContainer(c.Id).stop({ t: 2 });
        killed.push(short);
      } catch (e) {
        try { await docker.getContainer(c.Id).kill(); killed.push(short); } catch (e2) {}
      }
    }
  } catch (e) {
    console.warn('[force-stop] 清理 runner job 容器失败:', e.message);
  }
  if (killed.length) console.log(`[force-stop] 已停掉遗留的 job 容器: ${killed.join(', ')}`);
  return killed;
}
...
app.post('/api/gitea/sync/force-stop', async (req, res) => {
  try {
    ...
    if (runs.length === 0) {
      // Gitea 侧看不到运行中的任务，但 runner 上可能还挂着没退干净的 job 容器 —— 顺手清一次，
      // 免得用户看到「已经在下载/写盘」，点强制停止却什么也没发生。
      const leftovers = await cleanupRunnerJobContainers();
      // 进度文件可能还僵在 running（脚本被强杀时不会写终态），一并收尾，页面才不会一直显示「正在下载」
      const settled = await finalizeSyncProgress('cancelled', '强制停止：没有运行中的任务，已清理进度状态');
      if (leftovers.length) {
        await appendSyncHistory('cancelled', `强制停止：清掉 ${leftovers.length} 个遗留任务容器（${leftovers.join(', ')}）`);
        return res.json({
          ok: true,
          message: `Gitea 上没有运行中的任务，已清掉 ${leftovers.length} 个遗留任务容器`,
          killedJobs: leftovers,
        });
      }
      return res.json({
        ok: true,
        message: settled
          ? '没有正在运行的同步任务（已清理上次遗留的运行中状态）'
          : '没有正在运行的同步任务',
        progressSettled: !!settled,
      });
    }
    ...
    if (!stopped) {
      ...
      // runner 停了，它起的 job 容器还活着 —— 一并收掉，否则同步脚本会继续跑完
      killedJobs = await cleanupRunnerJobContainers();
    ...
    const jobTxt = killedJobs.length ? `，并清掉 ${killedJobs.length} 个遗留任务容器` : '';
    await finalizeSyncProgress('cancelled', `强制停止 ${runs.length} 个同步任务 (IDs: ${runIds.join(',')})`);
    await appendSyncHistory('cancelled', `强制停止 ${runs.length} 个同步任务 (IDs: ${runIds.join(',')})${jobTxt}`);
    
    res.json({ 
      ok: true, 
      message: stopped 
        ? `已通过 CANCELTASK 停止 ${runs.length} 个任务` 
        : `已强制停止 ${runs.length} 个任务（runner 已重启${jobTxt}）`,
      stopped: runIds,
      killedJobs,
...
app.post('/api/gitea/sync/sync-version', keycloak.protect('realm:ai-platform-admin'), async (req, res) => {
  try {
    ...
    if (resp.status === 204) {
      res.json({ ok: true, message: `已触发 ${ver} 同步任务（Gitea Action），请在更新历史中查看进度` });
    } else {
      const txt = await resp.text().catch(() => '');
      res.status(resp.status).json({ error: `触发失败 (HTTP ${resp.status}) ${txt}` });
    }
...
// 走 withDshDb（同一个串行队列 + 原子落盘）：历史写入和版本表写入会同时发生，
// 各自「读全库 → 整库回写」的话一定会互相覆盖。
async function appendSyncHistory(status, detail, version = '') {
  try {
    await withDshDb((db) => {
      db.run('INSERT INTO sync_history (version, status, detail) VALUES (?, ?, ?)',
...
// update-server 的 nginx webroot（宿主 ./dsh-updates）同时也挂进了 admin-portal
// （docker-compose.yml 里的 ./dsh-updates:/update-store），所以「手动创建版本」上传的安装包
// 直接写这里的 dsh/<ver>/ 就等于落进了 nginx 的 /usr/share/nginx/html/dsh/<ver>/。
// 不用 Docker tar API 是因为传上百 MB 的安装包会 Broken pipe（sync_download.py 已踩过这个坑）。
const DSH_UPDATE_ROOT = process.env.DSH_UPDATE_ROOT || '/update-store';
const DSH_STORE_DIR = path.join(DSH_UPDATE_ROOT, 'dsh');

// 平台 → 更新服务器上的标准文件名。自动同步（sync_download.py / syncVersionsFromServer）
// 就是靠文件名里的 windows / mac-arm64 / mac 关键字识别平台的，手动上传的包必须统一成这套命名，
// 否则「已同步版本」里会缺平台、客户端下载页也点不出对应安装包。
const DSH_PLATFORMS = {
  'windows-x64': { file: 'dsh-desktop-windows-x64-setup.exe', ext: '.exe', label: 'Windows x64' },
  'mac-x64':     { file: 'dsh-desktop-mac-x64.dmg',           ext: '.dmg', label: 'macOS x64 (Intel)' },
  'mac-arm64':   { file: 'dsh-desktop-mac-arm64.dmg',         ext: '.dmg', label: 'macOS ARM64 (Apple Silicon)' },
};

// 版本号按数字段比较（'v0.10.0' 应当 > 'v0.9.0'）。
// 注意 SQLite 的 `ORDER BY version DESC` 是纯字符串排序，会把 v0.10.0 排到 v0.9.0 前面，
// 直接拿它做剪枝就会误删真正的新版本 —— 凡是要判断「谁更新」的地方都必须走这两个函数。
...
function sortVersionsDesc(list) {
  return (list || []).slice().sort((a, b) => {
    ...
    return (y[0] - x[0]) || (y[1] - x[1]) || (y[2] - x[2]) || String(b).localeCompare(String(a));
  });
}

// 「最多保留几个版本」的真源是 dsh-sync 仓库里的 sync-config.json（界面上「保留版本数」可改），
// 读不到才回退环境变量 DSH_KEEP_VERSIONS。结果缓存 30 秒，免得每次剪枝都打一次 Gitea API。
let _keepVersionsCache = { value: 0, at: 0 };
async function getKeepVersions() {
  const now = Date.now();
  if (_keepVersionsCache.value > 0 && (now - _keepVersionsCache.at) < 30000) {
    return _keepVersionsCache.value;
  }
  let keep = DSH_KEEP_VERSIONS;
  try {
    const r = await giteaApi(`/api/v1/repos/${GITEA_ADMIN_USER}/dsh-sync/contents/sync-config.json?ref=main`);
    const cfg = JSON.parse(Buffer.from(r.data.content, 'base64').toString('utf8'));
    const k = parseInt(cfg.keep_releases, 10);
    if (k >= 1 && k <= 20) keep = k;
  } catch (e) {
    console.warn('[versions] keep_releases 读取失败，回退 DSH_KEEP_VERSIONS =', DSH_KEEP_VERSIONS, '-', e.message);
  }
  _keepVersionsCache = { value: keep, at: now };
  return keep;
}

// 把 sync-progress.json 收成终态。同步脚本被强杀（强制停止 / runner 重启 / 超时清理）时
// 没机会写终态，进度文件会僵在 status=running，页面就会一直显示「正在下载安装包」。
// 进度文件正好在 /update-store 挂载里，直接写文件即可（比走 docker exec 稳）。
// 只在它确实还是 running 时才动手，已经收尾过的保持原样。
async function finalizeSyncProgress(status, detail) {
  const p = path.join(DSH_UPDATE_ROOT, 'dsh', 'sync-progress.json');
  let cur = {};
  try {
    if (fs.existsSync(p)) cur = JSON.parse(fs.readFileSync(p, 'utf8')) || {};
  } catch (e) { cur = {}; }
  if (cur.status && cur.status !== 'running') return null;   // 已是终态，不用动

  const logs = Array.isArray(cur.logs) ? cur.logs.slice(-200) : [];
  const t = new Date();
  const hhmmss = [t.getHours(), t.getMinutes(), t.getSeconds()]
    .map(n => String(n).padStart(2, '0')).join(':');
  logs.push({ time: hhmmss, level: 'warn', msg: detail });

  const next = {
    ...cur,
    phase: status === 'cancelled' ? 'cancelled' : 'partial',
    status,
    detail,
  ...
  try {
    ...
    console.log(`[sync-progress] 已收尾为 ${status}：${detail}`);
  } catch (e) {
    console.warn('[sync-progress] 收尾失败:', e.message);
  }
  return next;
}
...
async function syncVersionsFromServer() {
  try {
    ...
    // 2. 更新数据库（走 withDshDb 的串行队列，避免和「删除版本」等写操作互相覆盖）
    await withDshDb((db) => {
      // 获取数据库中现有版本
    ...
    // 3. 剪枝统一交给 pruneVersions（它自己开库，并会一并删掉 update-server 上的目录）
    await pruneVersions();
  } catch (e) {
...
//
// 这里守着两个不变量（2026-09-14 查「删除已同步版本报错」时补齐的）：
// 1) 串行化：每次写都是「读全库 → 改 → 整库回写」。两个写操作并发时，后落盘的会拿自己
//    那份旧快照把前一个的改动整个盖掉 —— 典型症状是刚删掉的版本又冒回来、
//    或者刚写的版本记录丢失。所以所有写路径排在同一条 Promise 链上，一次只跑一个。
// 2) 原子落盘：先写 .tmp 再 rename 覆盖。直接 writeFileSync 有「写到一半进程挂掉」
//    和「读方恰好读到半截文件」两个窗口，后者会让 sql.js 抛 file is not a database。
const DSH_DB_PATH = process.env.DSH_DB_PATH || '/app/dsh-updates/db/dsh.db';
let _dshDbQueue = Promise.resolve();
function withDshDbLock(fn) {
  const next = _dshDbQueue.then(fn, fn);
  _dshDbQueue = next.then(() => {}, () => {});   // 队列不因单次失败而中断
  return next;
}
async function withDshDb(fn) {
  return await withDshDbLock(async () => {
    const initSqlJs = require('sql.js');
    const SQL = await initSqlJs();
    const db = new SQL.Database(fs.readFileSync(DSH_DB_PATH));
    try {
      const result = await fn(db);
      const tmpPath = `${DSH_DB_PATH}.tmp`;
      fs.writeFileSync(tmpPath, Buffer.from(db.export()));
      fs.renameSync(tmpPath, DSH_DB_PATH);       // 同目录 rename，POSIX 下是原子替换
      return result;
    } finally {
...
// 返回 { version, existed }：existed 让调用方区分「真的删掉了一条记录」和「本来就是空的」，
// 后者不该往「更新历史」里写记录（否则会出现一串「删除版本 vX.Y.Z（0 个文件）」的噪音）。
async function deleteVersionRow(version) {
  const ver = normalizeVersion(version);
  if (!ver) throw new Error(`版本号格式不合法，应为 vX.Y.Z（收到 "${version}"）`);
  return await withDshDb((db) => {
    const found = db.exec('SELECT 1 FROM versions WHERE version = ?', [ver]);
    const existed = !!(found.length && found[0].values.length);
    db.run('DELETE FROM version_files WHERE version = ?', [ver]);
    db.run('DELETE FROM versions WHERE version = ?', [ver]);
    return { version: ver, existed };
  });
}

// update-server 上版本目录的根 = nginx webroot（docker-compose 里 ./dsh-updates:/usr/share/nginx/html）
const DSH_NGINX_ROOT = process.env.DSH_NGINX_ROOT || '/usr/share/nginx/html';

// 单引号 shell 转义。版本号已经过 normalizeVersion 白名单校验，这里再兜一层，
// 免得以后有人把别的字符串拼进路径里。
function shq(s) { return `'${String(s).replace(/'/g, `'\\''`)}'`; }

// 删掉 update-server 上的版本目录，返回 { removed, leftovers, stderr }。
//
// 为什么不直接 `rm -rf` 一把梭：这个目录其实是 **Windows 宿主目录**（./dsh-updates）挂进容器的。
// Windows 侧只要有进程还攥着安装包的句柄 —— 正在安装/正在运行的安装包、杀毒实时扫描、
// SmartScreen 复检、Explorer 预览、Windows Search 索引 —— Linux 这边 rm 就会
// "Permission denied"，整个 rm -rf 也就删不干净。
// 老实现一轮 rm 失败就直接回 500「目录未能删除，请稍后重试」：管理员反复点还是失败，
// 又看不到到底是谁占着，成了死结。
// 现在：删不掉就退避重试；仍然删不掉，把残留文件名如实交回调用方，由它决定是否降级处理。
async function removeVersionDir(ver, { attempts = 3, delayMs = 800 } = {}) {
  const dir = `${DSH_NGINX_ROOT}/dsh/${ver}`;
  let stderr = '';
  let leftovers = [];
  for (let i = 1; i <= attempts; i++) {
    const r = await dockerExec(UPDATE_CONTAINER, ['sh', '-c',
      `rm -rf -- ${shq(dir)} 2>&1; echo '===STATE==='; ` +
      `if [ -e ${shq(dir)} ]; then echo EXISTS; ls -1A ${shq(dir)} 2>/dev/null; else echo GONE; fi`]);
    const out = String(r.stdout || '') + String(r.stderr || '');
    const [head, tail = ''] = out.split('===STATE===');
    stderr = head.trim();
    if (/^\s*GONE\b/m.test(tail)) return { removed: true, leftovers: [], stderr };
    leftovers = tail.split('\n').map(s => s.trim()).filter(s => s && s !== 'EXISTS');
    // 短暂退避：多数情况是杀毒/索引正在扫这个包，等一下它就松手了
    if (i < attempts) await new Promise(res => setTimeout(res, delayMs * i));
  }
  return { removed: false, leftovers, stderr };
}

// 只保留最新的 keep 个版本，其余连数据库记录和 update-server 上的目录一起删掉。
// keep 不传就取「保留版本数」的当前配置。返回被删掉的版本号（新 → 旧），供调用方记历史。
async function pruneVersions(keepCount) {
  const keep = keepCount || await getKeepVersions();
  const removed = await withDshDb((db) => {
    const all = [];
    const st = db.prepare('SELECT version FROM versions');
    while (st.step()) all.push(st.getAsObject().version);
    st.free();
    const ordered = sortVersionsDesc(all);
    if (ordered.length <= keep) return [];
    const drop = ordered.slice(keep);
    for (const v of drop) {
      db.run('DELETE FROM version_files WHERE version = ?', [v]);
      db.run('DELETE FROM versions WHERE version = ?', [v]);
    }
    return drop;
  });
  for (const v of removed) {
    try {
      const r = await removeVersionDir(v, { attempts: 2, delayMs: 500 });
      if (!r.removed) {
        console.warn(`[versions] ${v} 的目录没能删净，仍残留：${r.leftovers.join(', ') || '(未知)'}${r.stderr ? ' | ' + r.stderr : ''}`);
      }
    } catch (e) {
      console.warn(`[versions] 无法删除 ${v} 的目录:`, e.message);
    }
  }
  if (removed.length) {
    console.log(`[versions] 超出保留数量(${keep})，已删除：${removed.join(', ')}`);
  }
  return removed;
}

// 「服务端最新版本号」的唯一出处：SQLite 的 versions 表（走 generateVersionsJson 的缓存路径）。
//
// 以前这个值是从 update-server 上的 `dsh-updates/version.txt` 这个文件读的，但那个文件的维护者
// 一直是空的（没有任何代码写它，曾长期停在 v0.7.2）。2026-09-14 起统一改成读数据库，
// version.txt 弃用 —— 别再把这个文件接回来（界面显示、可用性检查、产品统计都走这里）。
async function latestVersionLabel() {
  try {
    const { versions } = await generateVersionsJson();
    return (versions[0] && versions[0].version) || '—';
  } catch (e) {
    console.warn('[versions] 读取最新版本失败:', e.message);
    return '—';
  }
}
...
async function generateVersionsJson() {
  try {
    ...
    // 查询版本（顺序交给 sortVersionsDesc：SQLite 的字符串 DESC 会把 v0.10.0 排到 v0.9.0 后面）
    const rows = [];
    const stmt = db.prepare('SELECT version, date FROM versions');
    while (stmt.step()) rows.push(stmt.getAsObject());
    stmt.free();
    const byVer = new Map(rows.map(r => [r.version, r]));

    const versions = [];
    for (const ver of sortVersionsDesc(rows.map(r => r.version))) {
      ...
      filesStmt.free();
      versions.push({ version: ver, date: (byVer.get(ver) || {}).date, files });
    }
    db.close();
...
app.put('/api/gitea/sync/versions/:ver', keycloak.protect(), protectAdmin('gitea'), async (req, res) => {
  try {
    ...
    const r = await withDshDb((db) => {
      ...
      if (!found.length || !found[0].values.length) throw new Error(`${ver} 不在本地部署版本中`);
...
// 唯一的例外：安装包被 Windows 侧进程攥着（正在装/正在跑、杀毒扫描），重试也删不掉 ——
// 这时先回 409 把残留文件名摊开，管理员二次确认后可以带 ?force=1 只移除记录（见下方）。
app.delete('/api/gitea/sync/version/:ver', keycloak.protect(), protectAdmin('gitea'), async (req, res) => {
  const ver = normalizeVersion(req.params.ver);
  if (!ver) return res.status(400).json({ error: '版本号格式不合法，应为 vX.Y.Z' });
  // force=1 只在「文件确实被占用删不掉」之后、由管理员二次确认才会带上（见前端 giteaDeleteVersion）。
  // 语义是：文件删不掉就先不删，但把列表记录去掉，别让管理员卡在一个永远删不掉的条目上。
  ...
  try {
    ...
    let before;
    try {
      before = await dockerExec(UPDATE_CONTAINER, ['sh', '-c',
        `ls -1 ${shq(dir)} 2>/dev/null; echo '---SIZE---'; du -sk ${shq(dir)} 2>/dev/null | awk '{print $1}'`]);
    } catch (e) {
      return res.status(503).json({ error: `${UPDATE_CONTAINER} 容器不可达，未删除任何内容：${e.message}` });
    }
    const parts = String(before.stdout || '').split('---SIZE---');
    const removedFiles = parts[0].split('\n').map(s => s.trim()).filter(Boolean);
    const freedKb = parseInt((parts[1] || '').trim(), 10) || 0;

    // 2. 删除整个版本目录（所有平台安装包都在这一个目录里）。带重试，见 removeVersionDir。
    const rm = await removeVersionDir(ver);
    if (!rm.removed) {
      const leftTxt = rm.leftovers.join('、') || '(未知文件)';
      // 以前这里只回一句「请稍后重试」，服务端连日志都没有，线上根本查不出是谁占着文件
      console.warn(`[dsh-delete] ${ver} 删除未完成，残留：${leftTxt}${rm.stderr ? ' | ' + rm.stderr : ''}`);
      if (!force) {
        return res.status(409).json({
          error: `${ver} 的安装包正被占用，没能删除：${leftTxt}。` +
                 `常见原因是这个安装包正在被安装/运行，或被杀毒软件、Windows 索引实时扫描占用。` +
                 `关掉占用它的程序后重试；若只想把它从「本地部署版本」里去掉，会再问你一次是否可以只移除记录。`,
          version: ver, dir, leftovers: rm.leftovers, rmStderr: rm.stderr, canForce: true,
        });
      }
    }

    // 3. 删数据库记录（versions + version_files 是唯一权威源）
    const dbRow = await deleteVersionRow(ver);

    // 目录本来就不存在、库里也没有这条记录 → 这次点击其实什么都没删掉。
    // 这种「空删除」不写更新历史：否则列表里会攒出一串「删除版本 vX.Y.Z（0 个文件）」的噪音
    // （典型来源：列表还没刷新时又点了一次、或者脚本对着已经不存在的版本做幂等清理）。
    const nothingDeleted = rm.removed && removedFiles.length === 0 && !dbRow.existed;
    if (nothingDeleted) {
      console.log(`[dsh-delete] ${ver} 目录与版本记录都不存在，未写历史${rm.stderr ? ' | ' + rm.stderr : ''}`);
      invalidateVersionsCache();
      return res.json({ ok: true, noop: true, version: ver, message: `${ver} 已不存在，无需删除` });
    }

    // 4. 记历史（带版本号）+ 失效缓存 + 通知前端立即刷新
    const sizeTxt = freedKb > 0 ? `，释放约 ${(freedKb / 1024).toFixed(0)} MB` : '';
    const leftTxt = rm.removed
      ? ''
      : `；${rm.leftovers.length} 个文件被占用未删（${rm.leftovers.join('、')}），仅移除记录`;
    await appendSyncHistory('success', `删除版本 ${ver}（${removedFiles.length} 个文件${sizeTxt}${leftTxt}）`, ver);
    invalidateVersionsCache();
    broadcastSyncChange('version-delete');

    console.log(`[dsh-delete] ${ver} removed: ${removedFiles.length} file(s) from ${dir}` +
      (rm.removed ? '' : ` | 残留未删: ${rm.leftovers.join(', ')} (force)`));
    res.json({
      ok: true, version: ver, dir, removedFiles, freedBytes: freedKb * 1024,
      leftovers: rm.leftovers, forced: !rm.removed,
    });
  } catch (e) {
    // 以前这里的异常既不记日志也不带上下文，线上只能看到一个光秃秃的 500 message
    console.error(`[dsh-delete] ${ver} 删除失败:`, e.message);
    res.status(500).json({ error: `删除 ${ver} 失败：${e.message}`, version: ver, dir });
  }
});

// ═══════════════════════════════════════════
// 手动创建版本（GitHub 拉不动时的兜底通道）
// 分两步：① 逐个 PUT 安装包的原始二进制（接口直接流式落盘，不经 Docker tar API，
// 上百 MB 的包也不会 Broken pipe）；② 调一次 finalize 写版本表 + 记历史 + 按保留数量剪枝。
// 上传阶段完全不碰数据库，所以中途失败不会留下「列表里看得到、磁盘上没有」的脏数据。
// ═══════════════════════════════════════════
app.post('/api/gitea/sync/versions/manual-upload', keycloak.protect(), protectAdmin('gitea'), async (req, res) => {
  const ver = normalizeVersion(req.query.version);
  if (!ver) return res.status(400).json({ error: '版本号格式不合法，应为 vX.Y.Z' });
  const platform = String(req.query.platform || '');
  const spec = DSH_PLATFORMS[platform];
  if (!spec) return res.status(400).json({ error: `不支持的平台：${platform || '(空)'}` });

  const origName = String(req.query.filename || '').trim();
  if (origName && spec.ext && !origName.toLowerCase().endsWith(spec.ext)) {
    return res.status(400).json({ error: `${spec.label} 需要 ${spec.ext} 文件，收到的是「${origName}」` });
  }

  const destDir = path.join(DSH_STORE_DIR, ver);
  const finalPath = path.join(destDir, spec.file);
  const tmpPath = finalPath + '.part';

  try {
    // 挂载点本身必须是存在的目录，否则 mkdirSync 只会在容器内凭空造目录、文件其实落不到 nginx 那边
    if (!fs.existsSync(DSH_UPDATE_ROOT)) {
      throw new Error(`更新服务器挂载点 ${DSH_UPDATE_ROOT} 不存在（检查 admin-portal 的 ./dsh-updates:/update-store 挂载）`);
    }
    // 覆盖已有版本时（前端只给第一个文件带 reset=1）先把目录清空，避免旧安装包残留在版本目录里
    if (String(req.query.reset || '') === '1' && fs.existsSync(destDir)) {
      fs.rmSync(destDir, { recursive: true, force: true });
    }
    fs.mkdirSync(destDir, { recursive: true });
  } catch (e) {
    return res.status(500).json({ error: `无法准备更新服务器目录：${e.message}` });
  }

  const MAX_BYTES = 2 * 1024 * 1024 * 1024; // 单个安装包 2GB 上限
  let bytes = 0;
  let aborted = false;
  ...
  req.on('data', (chunk) => {
    ...
    if (bytes > MAX_BYTES && !aborted) {
      ...
      cleanup();
      if (!res.headersSent) res.status(413).json({ error: '单个安装包不能超过 2GB' });
    }
  });
  req.on('aborted', () => { aborted = true; try { ws.destroy(); } catch (e) {} cleanup(); });
  req.on('error', () => { aborted = true; try { ws.destroy(); } catch (e) {} cleanup(); });
  ws.on('error', (e) => {
    if (aborted) return;
    aborted = true;
    cleanup();
    if (!res.headersSent) res.status(500).json({ error: `写入安装包失败：${e.message}` });
  });
  ws.on('finish', () => {
    ...
    try {
      ...
      cleanup();
      return res.status(500).json({ error: `安装包落地失败：${e.message}` });
    }
    console.log(`[manual-version] ${ver} / ${platform} ← ${origName || spec.file} (${(bytes / 1024 / 1024).toFixed(1)} MB)`);
    res.json({ ok: true, version: ver, platform, filename: spec.file, bytes });
  });
  req.pipe(ws);
});

// 收尾：把上传好的安装包登记进版本表、记一条「手动创建」历史，并执行保留数量剪枝
app.post('/api/gitea/sync/versions/manual-finalize', keycloak.protect(), protectAdmin('gitea'), async (req, res) => {
  try {
    const ver = normalizeVersion((req.body || {}).version);
    if (!ver) return res.status(400).json({ error: '版本号格式不合法，应为 vX.Y.Z' });

    const destDir = path.join(DSH_STORE_DIR, ver);
    if (!fs.existsSync(destDir)) {
      return res.status(400).json({ error: `更新服务器上没有 ${ver} 目录，请先上传安装包` });
    }

    const files = {};
    const missing = [];
    for (const [platform, spec] of Object.entries(DSH_PLATFORMS)) {
      if (fs.existsSync(path.join(destDir, spec.file))) files[platform] = spec.file;
      else missing.push(spec.label);
    }
    if (Object.keys(files).length === 0) {
      return res.status(400).json({ error: `${ver} 目录里没识别到任何安装包` });
    }

    // 日期取目录 mtime，跟自动同步扫描出来的口径保持一致（容器 TZ=Asia/Shanghai）
    let date = new Date().toISOString().slice(0, 10);
    try {
      const d = new Date(fs.statSync(destDir).mtime);
      date = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
    } catch (e) {}

    await upsertVersion(ver, date, files);

    const platformTxt = Object.keys(files).map(p => DSH_PLATFORMS[p].label).join('、');
    const missTxt = missing.length ? `，缺 ${missing.join('、')}` : '';
    await appendSyncHistory('manual',
      `手动创建版本 ${ver}（${Object.keys(files).length} 个安装包：${platformTxt}${missTxt}）`, ver);

    const pruned = await pruneVersions();
    invalidateVersionsCache();
    broadcastSyncChange('version-manual');

    console.log(`[manual-version] ${ver} 入库完成（${Object.keys(files).length} 个文件）${pruned.length ? '，超出保留数量已删：' + pruned.join(', ') : ''}`);
    res.json({ ok: true, version: ver, date, files, missing, pruned });
  } catch (e) {
    console.error('[manual-version] finalize error:', e.message);
    res.status(500).json({ error: e.message });
  }
});
...
// opts.timeoutMs：命令超时。原先这里只有 stream.on('end') 一条出口——hijack 的流在
// 容器被强杀、Docker 守护进程重启等情况下可能只 close 不 end，Promise 就永远不 settle，
// 调用方的 HTTP 请求跟着一起挂死（页面表现为「点了没反应」，服务器端一行日志都没有）。
// 现在补 close 作为兜底出口 + 超时 reject，宁可报一个明确的超时错，也不要静默挂住。
function dockerExec(containerName, cmd, opts = {}) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      stream.on('end', () => finish(resolve, { stdout, stderr }));
      stream.on('close', () => finish(resolve, { stdout, stderr }));
      stream.on('error', (e) => finish(reject, e));
      timer = setTimeout(
        () => finish(reject, new Error(`容器 ${containerName} 执行命令超时（${timeoutMs} ms）：${cmd[cmd.length - 1]}`)),
        timeoutMs
      );
...
app.get('/api/update/overview', keycloak.protect(), async (req, res) => {
  try {
    // 版本号来自 SQLite 版本表（latestVersionLabel），不再读已弃用的 version.txt；
    // 文件清单仍然扫目录，因为「有多少个包、什么时候落的盘」是磁盘事实
    const [version, statRes] = await Promise.all([
      latestVersionLabel(),
...
const availabilityTestDefs = [
  ...
  { id: 'update-server', name: '更新服务器', run: async () => {
      return `DSH Desktop 版本 ${await latestVersionLabel()}`;
    } },
...
async function collectProductStatus() {
  ...
  await Promise.all([
    ...
    add('update', async () => `DSH Desktop ${await latestVersionLabel()}`),
...
/* auto-fit 而不是 auto-fill：auto-fill 会把空轨道也留着，3 个安装包就只占最左边几条轨道，
   看着全堆在左侧；auto-fit 会把空轨道收掉，现有的卡片平分整行。
   再给单卡设上限 + 居中，免得只同步了一个平台（或只有一个安装包）时一张卡拉满整幅。 */
.dc-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 18px; justify-items: center; }
.dc-card { width: 100%; max-width: 440px; border: 1px solid var(--dc-border); border-radius: 16px; padding: 26px 20px; background: var(--dc-card); box-shadow: 0 1px 2px rgba(16,24,40,.05); text-align: center; transition: transform .16s ease, box-shadow .16s ease, border-color .16s ease; }
