...
#    .\ghost-content-import.ps1 -ServerAddr "192.168.1.10"            # 指定发布地址（IP）
#    .\ghost-content-import.ps1 -ServerAddr "portal.company.com"      # 指定发布地址（域名）
#    .\ghost-content-import.ps1 -SeedPath ".\translated.json"         # 用翻译后的 seed
#
#  说明：
#    seed 里的 <服务器IP> 占位符会被替换成 -ServerAddr 传入的对外发布地址（IP 或域名）。
#    翻译由部署 Agent 完成（先问发布地址 + 语言，非中文则翻译 title/html 等字段后再调本脚本）。
# ============================================================
param(
...
Step "2/2 执行导入（发布地址: $ServerAddr）"
$out = docker exec $Container node /tmp/ghost-content-import.js /tmp/content.json $ServerAddr 2>&1
if ($LASTEXITCODE -ne 0) { Fail "导入失败: $out" }
Ok ($out | Select-Object -Last 1)

Write-Host "`n完成。打开 http://$ServerAddr`:8090 查看首页（示例新闻已导入）。" -ForegroundColor Green
