...
| Cliente | DSH Desktop (Windows/macOS) · Linux (próximamente) | Cliente de escritorio de IA local — DSH Desktop en Windows/macOS; versión para Linux próximamente |
| Distribución de cliente | Update Server | Alojamiento de instaladores del cliente de escritorio y actualización automática (DSH Desktop para Windows/macOS) |
...
**Dify** — plataforma de aplicaciones de IA · **Mercado de MCP/Skills** — herramientas y skills con un solo clic · **DSH Desktop** — cliente de IA de escritorio
...
- **Corregido: DSH Desktop sync Broken pipe** — reescritura de `deploy_file_to_nginx` para usar `wget` dentro del contenedor update-server en lugar de la API Docker tar, corrigiendo el problema de Broken pipe en archivos de instalación de más de 160 MB
- **Corregido: actualización de la página Ghost DSH** — el contenedor admin-portal ahora instala `python3` al inicio (necesario para el script de actualización de la base de datos Ghost)
- **Corregido: análisis JSON de Docker API** — `docker_exec_read` ahora analiza correctamente las respuestas chunked transfer encoding
- **Corregido: inconsistencia del nombre de campo sync-version** — unificación de `resync_version` → `sync_version` (admin-portal, Gitea workflow, frontend)
- **Mejorado: `export-images.ps1`** — añadido parámetro `-Version` para nombres de archivos de imagen con número de versión
- **Mejorado: `publish.ps1`** — eliminada la lógica de分割 de imágenes Gitee (Gitee usa 123pan para distribución), extracción automática del número de versión desde el tag

### v1.05（2026-09-02）

- **Corregido: bucle de redirección Keycloak SSO** — mejora de la lógica de eliminación del parámetro `iss` para borrar solo en solicitudes no-callback, permitiendo que el flujo de inicio de sesión OAuth se complete normalmente
- **Corregido: nombre del contenedor de base de datos Dify** — corrección de `docker-db_postgres-1` → `dify-db_postgres-1` en admin-portal
- **Nuevo: botones de reinicio de pruebas de disponibilidad** — cada elemento de prueba ahora muestra un botón de reinicio con un clic para su contenedor Docker asociado
- **Nuevo: guía de implementación sin conexión** — añadidas instrucciones para descargar e importar paquetes de imágenes preconstruidos desde Releases
- **Mejorado: guías de implementación y scripts** — actualización de guías de implementación Windows/Linux, scripts de respaldo/restauración

### v1.04（2026-08-30）

- **Mejorado: reestructuración de la barra lateral del Centro de administración** — grupos renombrados a「Servicios de aplicación / Infraestructura de plataforma / Supervisión de operaciones / Administración del sistema」；LiteLLM movido a Servicios de aplicación, Keycloak movido a Infraestructura de plataforma
- **Corregido: bucle de redirección del parámetro `iss`** — añadido middleware para eliminar los parámetros de consulta `iss` antes del procesamiento del middleware Keycloak
- **Mejorado: configuración de LiteLLM** — actualizada a los últimos parámetros recomendados
- **Mejorado: internacionalización del README** — actualizadas las 8 versiones lingüísticas
...
### 🎯 Habilidad de operación de IA lista para usar — descargar y desplegar

> El repositorio incluye ahora una **habilidad de operación lista para usar** ([`AIOperation/agent/`](../AIOperation/agent/SKILL.md)) que convierte a cualquier agente de IA (WorkBuddy, OpenClaw, Microsoft Scout o equivalente) en un operador completo de la plataforma — **sin configuración específica del servidor**. Sin IP, sin contraseñas, sin rutas codificadas: las credenciales se leen de `.env`, las rutas se resuelven automáticamente, por lo que funciona en **cualquier máquina** donde se despliegue la plataforma.

**Qué cubre la habilidad** (toda la gestión diaria): comprobaciones de salud con un comando (41 contenedores × 9 fases), inicio/parada/reinicio de contenedores y diagnóstico de registros, cambios de configuración, todo el AI Admin Center — administradores y roles, sincronización Keycloak/AD, canales/tokens/costes de NewAPI, sincronización de Gitea, portal Ghost, Dify, MCP Gateway, monitorización/alertas/registros/PII, pruebas de disponibilidad, informes, copia de seguridad y restauración, alertas de IM — la gestión nativa de cada producto de terceros (dominios/roles/clientes Keycloak, canales/tokens NewAPI, modelos/usuarios LiteLLM, aplicaciones/bases de conocimiento Dify, contenidos/temas Ghost, repos/CI Gitea, puerta de enlace MCP, paneles/usuarios Grafana, proyectos/claves Langfuse, Prometheus/Alertmanager/Loki, Update Server), además de lanzamientos de versiones, limpieza de disco y solución de problemas.

**Descarga y despliegue (3 pasos):**

1. **Consíguela** — clona el repositorio o descarga la carpeta `AIOperation/` desde GitHub / Gitee:
   ```bash
   git clone https://github.com/sdlyxianchao/AIAllInOne
   # la habilidad está en: AIAllInOne/AIOperation/agent/
   ```
2. **Instálala** — copia la carpeta en el directorio de habilidades de tu agente (WorkBuddy: `~/.workbuddy/skills/ai-all-in-one-deploy-ops/`; otros agentes siguen su propia convención):
   ```bash
...
| Stage 5 | **Cadena LLM completa** : estado de canales NewAPI + una petición real en nombre de DSH Desktop y Dify (NewAPI → LiteLLM → DeepSeek) | `curl /v1/chat/completions` |
...
| Stage 8 | **Prerrequisitos de login DSH Desktop / Dify** : NewAPI disponible + Dify inicializado | curl + psql |
