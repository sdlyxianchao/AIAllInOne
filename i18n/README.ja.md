...
- **修正：DSH Desktop同期Broken pipe** — `deploy_file_to_nginx`をupdate-serverコンテナ内で`wget`を使用して直接ダウンロードするように書き換え、160MB以上のインストーラーファイルのBroken pipe問題を修正
- **修正：Ghost DSHページ更新** — admin-portalコンテナ起動時に`python3`をインストール（Ghostデータベース更新スクリプトに必要）
- **修正：Docker API JSON解析** — `docker_exec_read`がchunked transfer encodingレスポンスを正しく解析するよう修正
- **修正：sync-versionフィールド名不一致** — `resync_version` → `sync_version`に統一（admin-portal、Giteaワークフロー、フロントエンド）
- **改善：`export-images.ps1`** — バージョン番号付きイメージファイル名用の`-Version`パラメータを追加
- **改善：`publish.ps1`** — Giteeイメージ分割ロジックを削除（Giteeは123パンを使用して配布）、tagからバージョン番号を自動抽出

### v1.05（2026-09-02）

- **修正：Keycloak SSOリダイレクトループ** — `iss`パラメータ除去ロジックを改善し、非コールバックリクエスト時のみクリアし、OAuthログインフローを正常に完了できるように修正
- **修正：Difyデータベースコンテナ名** — admin-portalの`docker-db_postgres-1`を`dify-db_postgres-1`に修正
- **新規：可用性テスト再起動ボタン** — 各テスト項目に関連するDockerコンテナをワンクリックで再起動するボタンを表示
- **新規：オフラインデプロイメント説明** — Releasesからビルド済みイメージパッケージをダウンロードしてインポートする手順を追加
- **改善：デプロイメントガイドとスクリプト** — Windows/Linuxデプロイメントガイド、バックアップ/リストアスクリプトを更新

### v1.04（2026-08-30）

- **改善：管理センターサイドバー再編成** — グループ名を「アプリケーションサービス / プラットフォームインフラ / 運用監視 / システム管理」に変更；LiteLLMをアプリケーションサービスに移動、Keycloakをプラットフォームインフラに移動
- **修正：`iss`パラメータリダイレクトループ** — Keycloakミドルウェア処理前に`iss`クエリパラメータを除去するミドルウェアを追加
- **改善：LiteLLM設定** — 最新の推奨設定に更新
- **改善：README国際化** — 全8言語バージョンを更新

### v1.03（2026-08-28）

- **改善：管理センターサイドバー再編** — グループ名を「應用服務（アプリサービス）/ 平台基礎設施（基盤インフラ）/ 運維監控（運用監視）/ 系統管理（システム管理）」に変更；LiteLLM を應用服務に移動、Keycloak を平台基礎設施に移動、「企業 IM 告警」を「監控告警 + IM 通知」に統合、「PII 脱敏」スタンドアロン項目を削除（LiteLLM 名に統合）、「客戶端軟體同步」を「桌面客戶端管理」に改名
- **改善：PII 自動復元** — Presidio モードを `pre_call` から `["pre_call", "post_call"]` に変更；PII が LLM 応答で自動復元され、ユーザーは `<PERSON>` プレースホルダーを見なくなりました
- **修正：`iss` パラメータリダイレクトループ** — admin-portal server.js にミドルウェアを追加し、Keycloak ミドルウェア処理前に `iss` クエリパラメータを除去、トークン期限切れ後の無限リダイレクトループを防止
- **新規：DSH 同期スクリプト** — `dsh-sync-export/` ディレクトリを新設、DSH Desktop 自動更新同期用の Gitea Actions ワークフローを含む
...
### 🎯 すぐ使える AI 運用スキル——ダウンロード＆デプロイ

> リポジトリには**すぐ使える運用スキル**（[`AIOperation/agent/`](../AIOperation/agent/SKILL.md)）が同梱されており、どんな AI エージェント（WorkBuddy、OpenClaw、Microsoft Scout など）もプラットフォームのフル運用担当者に変えます——**サーバー固有の設定は一切不要**。IP・パスワード・パスはハードコードせず、認証情報は `.env` から読み取り、パスは自動解決するので、本プラットフォームをデプロイした**どのマシンでも**そのまま使えます。

**スキルのカバー範囲**（日常管理すべて）：ワンコマンド健康チェック（41 コンテナ × 9 ステージ）、コンテナの起動/停止/再起動とログ診断、設定変更、AI Admin Center 全体——管理者＆ロール、Keycloak/AD 同期、NewAPI チャネル/トークン/コスト、Gitea 同期、Ghost ポータル、Dify、MCP Gateway、監視/アラート/ログ/PII、可用性テスト、レポート、バックアップ＆リストア、IM アラート——さらに各サードパーティ製品のネイティブ管理（Keycloak のレルム/ロール/クライアント、NewAPI のチャネル/トークン、LiteLLM のモデル/ユーザー、Dify のアプリ/ナレッジベース、Ghost のコンテンツ/テーマ、Gitea のリポジトリ/CI、MCP ゲートウェイ、Grafana のダッシュボード/ユーザー、Langfuse のプロジェクト/キー、Prometheus/Alertmanager/Loki、Update Server）、さらにバージョンリリース、ディスククリーンアップ、トラブルシューティングまで。

**ダウンロード＆デプロイ（3 ステップ）：**

1. **入手** — リポジトリをクローン、または GitHub / Gitee から `AIOperation/` フォルダーをダウンロード：
   ```bash
