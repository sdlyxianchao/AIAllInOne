...
- **إصلاح: DSH Desktop sync Broken pipe** — إعادة كتابة `deploy_file_to_nginx` لاستخدام `wget` داخل حاوية update-server بدلاً من Docker API tar، مما يحل مشكلة Broken pipe على ملفات التثبيت التي يزيد حجمها عن 160 ميجابايت
- **إصلاح: تحديث صفحة Ghost DSH** — تقوم حاوية admin-portal الآن بتثبيت `python3` عند بدء التشغيل (مطلوب لسكريبت تحديث قاعدة بيانات Ghost)
- **إصلاح: تحليل JSON لـ Docker API** — يقوم `docker_exec_read` الآن بتحليل استجابات chunked transfer encoding بشكل صحيح
- **إصلاح: عدم تطابق اسم الحقل sync-version** — توحيد `resync_version` → `sync_version` (admin-portal، Gitea workflow، الواجهة الأمامية)
- **تحسين: `export-images.ps1`** — تمت إضافة معامل `-Version` لأسماء ملفات الصور مع رقم الإصدار
- **تحسين: `publish.ps1`** — تمت إزالة منطق تقسيم صور Gitee (يستخدم Gitee توزيع 123pan)، استخراج تلقائي لرقم الإصدار من الوسم

### v1.05（2026-09-02）

- **إصلاح: حلقة إعادة توجيه Keycloak SSO** — تحسين منطق إزالة معامل `iss` للمسح فقط في الطلبات غير callback، مما يسمح لإكمال تدفق تسجيل الدخول OAuth بشكل طبيعي
- **إصلاح: اسم حاوية قاعدة بيانات Dify** — تصحيح `docker-db_postgres-1` → `dify-db_postgres-1` في admin-portal
- **جديد: أزرار إعادة تشغيل اختبارات التوفر** — يعرض كل عنصر اختبار الآن زر إعادة تشغيل بنقرة واحدة لحاوية Docker المرتبطة به
- **جديد: دليل النشر بدون اتصال** — تمت إضافة تعليمات لتحميل واستيراد حزم الصور المبنية مسبقاً من Releases
- **تحسين: أدلة النشر والسكربتات** — تحديث أدلة النشر Windows/Linux، سكربتات النسخ الاحتياطي/الاستعادة

### v1.04（2026-08-30）

- **تحسين: إعادة هيكلة الشريط الجانبي لمركز الإدارة** — إعادة تسمية المجموعات إلى「خدمات التطبيق / البنية التحتية للمنصة / مراقبة العمليات / إدارة النظام」؛ تم نقل LiteLLM إلى خدمات التطبيق، Keycloak إلى البنية التحتية للمنصة
- **إصلاح: حلقة إعادة توجيه معامل `iss`** — تمت إضافة middleware لإزالة معاملات الاستعلام `iss` قبل معالجة middleware Keycloak
- **تحسين: إعدادات LiteLLM** — تحديث إلى أحدث الإعدادات الموصى بها
- **تحسين: تدويل README** — تحديث جميع الإصدارات اللغوية الـ8
