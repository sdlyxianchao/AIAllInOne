...
> **حالة المنصة**: Windows (Windows 11 + Docker Desktop) و**Linux** (Debian 12+ / Ubuntu 22.04+ / RHEL 9+ / Rocky 9+ / Fedora 39+) **قيد الاختبار الفعلي**. أما الخوادم عبر الإنترنت (`docker/`) ففي طور التخطيط — راجع [خارطة الطريق](#roadmap).
...
- ✅ v1.00 — الحزمة الكاملة: Windows + Linux (Debian/RPM) + Dify + النشر بدون اتصال + مركز إدارة الذكاء الاصطناعي + تفويض إداري متدرج + تنبيهات IM + التخزين المؤقت الدلالي (LiteLLM redis-semantic)
...
**جديد: دعم منصة Linux**
- دعم نشر كامل على Linux: توزيعات Debian (Debian 12+ / Ubuntu 22.04+) وتوزيعات RPM (RHEL 9+ / CentOS Stream 9+ / Rocky 9+ / Fedora 39+)
- حزم صور Docker مبنية مسبقًا للنشر بدون اتصال (6.2 جيجابايت الرئيسية + Dify)
- نصوص برمجية bash لـLinux: backup.sh، restore.sh، health-check.sh، ghost-setup.sh، dify-setup.sh
- نصوص استيراد لكل من Windows (PowerShell) وLinux (bash)

**جديد: منصة Dify لتطبيقات الذكاء الاصطناعي**
- دعم نشر مستقل لـ Dify مع Docker Compose منفصل
- سكربت تهيئة تلقائية: ينشئ محادثة افتراضية وقاعدة معرفة ومفتاح API
- تكامل Keycloak SSO لتسجيل الدخول الموحّد

**جديد: سير عمل مزامنة Gitea**
- تصدير وتوثيق نصوص مزامنة التحديث التلقائي لـ DeepChat
- سير عمل Gitea Actions لتوزيع عميل سطح المكتب تلقائيًا

**تحسين: أدلة النشر**
- دليل نشر Windows الإصدار 2: إضافة استيراد الصور بدون اتصال، المهام المجدولة، مرجع المنافذ السريع، سجل الإصلاحات الموثقة
- دليل نشر Linux (Debian/RPM): تكامل مماثل لدليل Windows
- جميع الأدلة تتضمن الآن استبيان مزود هوية Keycloak (7 أنواع من IdP) وتهيئة تلقائية لـ Admin REST API
- إضافة خطوات إعداد Dify التلقائي واستيراد مزامنة Gitea لجميع الأدلة

**تحسين: مركز إدارة الذكاء الاصطناعي**
- اختبارات التوافر لم تعد تُثبّت نموذج DeepSeek — بل تستعلم عن النماذج المتاحة ديناميكيًا عبر NewAPI
- يعمل مع أي خلفية LLM (DeepSeek، OpenAI، Claude، Ollama المحلي، إلخ)

**تحسين: صفحات GitHub**
- صفحة هبوط جديدة للمشروع على https://sdlyxianchao.github.io/AIAllInOne/
...
2. **التثبيت** — انسخ المجلد إلى دليل مهارات وكيلك (WorkBuddy: `~/.workbuddy/skills/ai-all-in-one-deploy-ops/`؛ الوكلاء الآخرون يتبعون اصطلاح مجلد المهارات الخاص بهم):
   ...
   ```
3. **الاستخدام** — افتح الوكيل في دليل النشر واسأل مباشرة، مثل «شغّل الفحص الصحي»، «انسخ المنصة احتياطيًا»، «لماذا تعطل Ghost؟»، «أصدر v1.00». تقرأ المهارة بيانات الاعتماد من `.env` بنفسها — لا تحتاج أبدًا إلى لصق كلمات المرور، وتتكيف مع أي جهاز تشير إليه.
